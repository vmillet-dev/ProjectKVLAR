...
void AKUPDebugHUD::DrawLocalPlayerPanel()
{
	...
	auto ElementColor = [Settings](const FSpellDefinition& Spell) -> FLinearColor
	{
		if (Settings && Spell.IsValid())
		{
			if (const UElementDefinition* Element = Settings->FindElement(Spell.Element))
			{
